...
do
  ...
  retry=false
  nodes=`nslookup tasks.$SERVICE_NAME 2>/dev/null | grep -v $(hostname) | grep Address | grep -v 127.0.0.1 | awk '{print $2}' | cut -d. -f1-4`
  for node in $nodes
  do
...
do
    ...
    do
        ...
        if nc -z "$node" 15672
        then
            rabbitnode=`nslookup $node 2> /dev/null | grep name | cut -d " " -f 3  | cut -d "." -f 1,2,3`
